...
### 축3 — 저자가 이미 산문으로 써 둔 인접성: **43쌍, 엣지는 0건**

> **정정 (2026-08-28, vnv 실측 대조 후)**: 아래를 처음에 "A-wave의 실제 수확물"이라고 썼으나
> **과한 표현이었다**. `ho:overlapsWith`의 뜻은 *설명 영역이 겹친다*인데 "Distinguished from X"는
> **차이 진술**이다(`pat-blackboard`: "MEDIUM을 제약하지 ordering을 제약하지 않는다"). 술어에
> 충실한 후보는 vnv가 별도로 실측한 **2쌍**(`gr-well-formed-skill↔ins-well-formed-skill` ·
> `chan-peer↔pat-peer-mesh` — 정의 문장이 실제로 겹침)이다. 아래 43쌍은 *다른 것*을 잰
> **혼동 인접성** 자산으로, 저작 여부는 별도 결정(`feedback/a-wave-annotation-content.md` 결정 2).
...
- **A1 (수정됨)**: `ho:overlapsWith` 저작은 **vnv 실측 2쌍**(정의가 실제로 겹치는 쌍)으로
  한정하는 것이 술어 의미에 맞다 — 위 정정 참조. 축3의 43쌍은 **혼동 인접성**이라 별개 자산이며
  저작 여부는 결정 2로 분리했다(권고: 저작하지 않고 필요 시 정규식 재측정 — 소비자 없는
  엣지를 넣지 않는다는 `anchorConfidence` 판단과 같은 기준).
