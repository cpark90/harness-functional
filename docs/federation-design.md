...
> **이송 표기 (2026-09)**: D1·D3·D4의 **일반 원칙**은 agentic-knowledge-base의
> 결정 청크로 이송되었다 — 카탈로그로 IRI를 파일로 해석해 union을 조립한다
> **d-0156**, 불변식은 union 위에서만 성립하므로 union을 검증한다 **d-0157**,
> 개체 이름공간을 소유자 세그먼트로 나눈다 **d-0158**, 생성물의 드리프트 가드
> **d-0159**. 이 문서는 그 원칙의 **이 연합에서의 실현**만 담는다: 실제 IRI,
> 실제 파일 경로, 실제 명령, 그리고 이 프로젝트의 배치인 D2.
...
원칙은 청크 d-0157(불변식은 union 위에서만 성립한다)에 있다. 이 연합에서의
실체는 세 보장 — 반-고아(도달성 + SHACL 연결성), 반-drift(통제 어휘), 조립가능
(`ho:requiresCapability`/`providesCapability` 충족, 청크 d-0014) — 이고, 그
셋은 전부 `ho:hasComponent` 간선이 저장소를 가로지르는 union에서만 판정된다.
그래서 D1(조립)과 D4(union 검증)가 한 설계의 양면이다.
...
**IRI는 위치 독립이다** (원칙: 청크 d-0156). 이 저장소의 증거: 2026-09
재배치에서 A-Box와 root가 저장소를 옮겼지만 IRI는 하나도 바뀌지 않았고
카탈로그의 경로만 바뀌었다.
...
CI가 `--check` 모드로 돌려 디스크와 어긋나면 실패시킨다. 원칙과 그 근거가 된
사건(카탈로그 누락으로 **부분** union만 로드된 채 검증이 통과)은 청크
**d-0159**에 있다.

### IRI 하위 네임스페이스 (D3)

원칙(소유자 세그먼트로 이름공간 분할, 개체 IRI ≠ 문서 IRI)은 청크 **d-0158**.
아래는 이 연합에서 쓰는 실제 네임스페이스와 접두사다.
...
union 게이트가 강제한다 — 좁은 게이트가 초록인 것은 파급이 없다는 증거가
아니다(청크 d-0157).
