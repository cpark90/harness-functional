...
단일 진실 공급원이라면, 이 문서는 `ho:` TTL 저작의 단일 진실 공급원이다.

> **경로 읽는 법.** 이 문서는 두 저장소에 걸쳐 적용된다.
> **TBox·shapes** = harness-functional의 `ontology/tbox/harness.ttl`·
> `ontology/shapes/harness-shapes.ttl`. **abox(개체)·레시피** = harness-concrete의
> `ontology/abox/core/<group>/*.ttl`·`recipes/<name>/`. 저장소를 명시하지 않은
> `ontology/abox/…` 표기는 **harness-concrete 기준**이다.
...
**고립 노드(orphan)는 예외가 아니라 build failure**다. `validate.py`의 SHACL 연결성 shape +
전역 reachability BFS가 이를 강제한다. 이 축은 **개체가 있는 union**에서만 실질적으로
걸린다 — 즉 **harness-concrete의 union 게이트**가 판정한다(harness-functional 단독 게이트는
개체가 0이라 공허하게 통과한다). (설계 원칙은 agentic-knowledge-base 청크 d-0014
"3대 실패 모드와 방어선" 참조.)
...
- **[지킴]** **저장된 그래프 전체(stored graph = TBox 층 + ABox 층, 두 저장소에 걸침)를
  context에 로드하지 않는다.** 요청 처리·composition은 항상
  `HARNESS_CATALOG=catalog-v001.xml python3 central/tools/retrieve.py "<request>"`
  (harness-concrete에서 실행)가 준 pack에서 시작한다
  (agentic-knowledge-base 청크 d-0013 "형식 저장·좁은 읽기").
...
- **[지킴]** 언어는 **한글 설명 + 용어는 영어**(CLAUDE 계열 언어 규칙). 단 `skos:prefLabel`·
  `definition` 같은 그래프 데이터 값은 **영어**로 쓴다(기존 core 부품과 일관 — 검색 대상 텍스트).

---

## 2. 개체(individual) 네이밍

> 이 절은 **개체 저작**의 규칙이므로 적용 현장은 **harness-concrete**다
> (`ontology/abox/core/**` + `recipes/<name>/`). harness-functional에는 개체가 없다.

중앙 core 부품 라이브러리의 접두사 규약을 그대로 따른다. IRI는 `id:` 네임스페이스,
소문자 kebab-case, `<kind>-<slug>` 꼴.
...
- **[지킴]** Turtle에서는 **prefix 바인딩으로만** 표현해 노드 본문을 그대로 둔다: 자기
  ...
  `@prefix core: <.../id/core/> .` + `core:<slug>` (중앙 부품 라이브러리의 개체를 재사용).
  두 prefix가
...
가독성을 위해 프레디킷을 **일정한 순서**로 나열한다(중앙 core 부품 라이브러리 관례):
...
- **[지킴]** `ho:Concept` 블록은 SKOS 관계(`skos:broader`/`topConceptOf`/`related`) **뒤 맨 끝**에
  ...
  중앙 커버리지는 `lint_uniformity.py`가 `id/core/` 스코프로 지킨다(도구는 harness-functional의
  `tools/`에 있고, 개체가 있는 harness-concrete에서 `central/tools/lint_uniformity.py`로 실행).
...
> 아래 **파일 배치·`owl:Ontology` 헤더·catalog 규칙은 개체 파일의 것**이므로 현장은
> **harness-concrete**다. 저장소 접두 없는 `ontology/abox/…`·`catalog-v001.xml`은 전부
> 그 저장소 기준으로 읽는다. harness-functional에는 `ontology/tbox/harness.ttl`(문서 IRI
> `.../schema`)와 `ontology/shapes/`만 있고, 그 catalog에는 schema 한 줄뿐이다.

- **[지킴]** prefix 블록은 파일 상단에, 기존 파일과 같은 순서(`ho`, `id`, `owl`, `rdf`,
  `rdfs`, `xsd`, `skos`, `dct`)를 따른다. `id:`는 abox에만. 다른 도메인 참조용 prefix(예: `core:`)는
...
- **[지킴]** 파일은 **DA-4 상위 taxonomy 그룹 디렉토리**에 물리적으로 놓는다 —
  harness-concrete의 `ontology/abox/core/<group>/<type>.ttl`. 그룹은 담는 타입의 상위계층을
  반영한다:
...
- **[지킴]** 스키마(클래스·프로퍼티)는 **harness-functional의 `ontology/tbox/`**, 개체는
  **harness-concrete의 `ontology/abox/`**. abox에서 새 클래스·프로퍼티를 선언하지 않고,
  tbox에 개체를 두지 않는다. 용어상 **TBox+shapes = ontology 층, ABox = knowledge graph
  층**이며, 2026-09 재배치로 두 층이 저장소 둘로 갈라졌다
  (`README.md`·`docs/federation-design.md`). IRI는 위치 독립이라 층이 갈라져도 개체 IRI·문서
  IRI는 그대로이고 catalog만 저장소별로 다르다.
...
- **[권장]** 텍스트·프레디킷이 여럿인 노드는 **여러 줄**, 프레디킷마다 4칸 들여쓰기:
  ...
  ```
- **[지킴]** 마지막 트리플은 `.`로 닫는다. 섹션은 기존 파일의 `#===== ... =====` 배너로 구분한다.
...
검사하는 게이트다 — 통과가 곧 "연결됨·타입 정합·drift 없음"의 증거다. **저작한 층에 맞는
게이트를 돌린다.**

**어휘·shape를 고쳤다면 (harness-functional):**

```bash
make validate      # = HARNESS_ROOT_ONTOLOGY=.../schema python3 tools/validate.py
                   # TBox 정합성·SHACL·라벨 중복. 개체가 0이라 연결성 축은 공허하게 통과한다.
```

**개체·레시피를 저작했다면 (harness-concrete — 연결성·capability 축이 실제로 걸리는 곳):**

```bash
HARNESS_CATALOG=catalog-v001.xml \
HARNESS_ROOT_ONTOLOGY=https://harness-ontology.dev/ontology \
python3 central/tools/validate.py                       # 반드시 PASS.
                                                        # FAIL이면 shape가 아니라 그래프를 고친다.
HARNESS_CATALOG=catalog-v001.xml \
python3 central/tools/retrieve.py "<새 노드가 답할 request>"   # 실제로 검색되는지 확인
```

어휘를 고쳤으면 **양쪽 다** 돌린다 — 좁은 게이트만 초록인 것은 개체 파급이 없다는 증거가
아니다(`HARNESS-ODD.md` 조건부 규정: TBox 변경 후 concrete catalog 재생성 + 전체 레시피 게이트).

> 환경 주의: 도구는 `rdflib`/`pyshacl`/`owlrl`가 있는 인터프리터로 실행해야 한다.
> 셸 기본 `python3`에 없으면 그 셋이 설치된 인터프리터로 실행한다(예: `/usr/bin/python3`).
