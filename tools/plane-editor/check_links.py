...
def _walk(root: str):
    ...
    두 가지를 지킨다.
      - **사이클**: 이미 방문한 실경로는 다시 내려가지 않는다. 이 하네스 저장소들에는
        저장소 밖을 가리키는 디렉토리 심링크가 실제로 있다 — harness-concrete의
        `central/`이 harness-functional 체크아웃을 가리킨다(2026-09 재배치 이전에는
        `staging/harness-recipes/central`이 같은 역할을 했다).
